...
public class UuidUtil {
    public static UUID validateUUIDType(String parameter) {
        UUID uuidText = new UUID(0L,0L);
        try {
            uuidText = UUID.fromString(parameter);
        } catch (Exception e) {
            throw new VocabuilderIllegalArgumentException("validation - the id provided is not valid", HttpStatus.BAD_REQUEST);
        }
        return uuidText;
    }
}
